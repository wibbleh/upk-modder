 ...
 *  of a JScrollPane.
 *  Source : http://www.camick.com/java/source/TextLineNumber.java
 *  Adapted by : Amineri
 *  Adaptation allows for display of memory position instead of line number.
 */
	...
	implements CaretListener, DocumentListener, PropertyChangeListener
{
	...
	private final static int LINE_PANE_HEIGHT = Integer.MAX_VALUE - 1000000;
	...
	// the associate ModTree used to drawn memory positions from
	...
	/**
	 * Extended constructor used to set ModTree to draw memory positions from
	 * @param component the related text component
	 * @param minimumDisplayDigits as in base constructor
	 * @param aTree the associate ModTree to draw memory/file positions from
	 */
	public TextLineNumber(JTextComponent component, int minimumDisplayDigits, ModTree aTree) {
		this(component, minimumDisplayDigits);
		this.tree = aTree;
	}
	...
	private void setPreferredWidth()
	{
		...
		if (lastDigits != digits)
		{
			...
			d.setSize(preferredWidth, LINE_PANE_HEIGHT);
			setPreferredSize( d );
			setSize( d );
		}
	}
	 ...
	 *  Adapted to alternatively display the memory position of the line from an associated ModTree model
	 */
	protected String getTextLineNumber(int rowStartOffset)
	{
		...
		if(true) {  // branch to display Memory position
			...
			int filePos = lineNode.getFilePosition();
				return String.format("%04X", filePos);
			} else {
				return "";
			}
		} else {  // original code branch to display line number
			Element root = component.getDocument().getDefaultRootElement();
